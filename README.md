...
수정할 수 있는 **관리자 편집 기능**(비밀번호 로그인)을 Vercel 서버리스 함수 + Vercel KV로
붙였습니다. 호스팅은 **Vercel**을 사용합니다(무료 요금제로 충분).
...
├── data/content.json      # 콘텐츠 기본값(seed) — Vercel KV가 비어있을 때 사용됨
...
Vercel KV에 저장된 내용이 우선 표시됩니다. 되도록 관리자 페이지 사용을 권장합니다.
...
## 5. 관리자 편집 기능 활성화 (Vercel KV + 비밀번호 설정)

관리자 페이지가 실제로 저장을 하려면 **Vercel KV(무료 데이터베이스)**와 **관리자 비밀번호**를
한 번만 연결해주면 됩니다.

1. Vercel 프로젝트 페이지 → **Storage** 탭 → **Create Database** → **KV** 선택 → 이름 입력 후 생성.
2. 생성된 KV를 이 프로젝트에 **Connect**합니다. (환경변수 `KV_REST_API_URL`, `KV_REST_API_TOKEN` 등이 자동으로 추가됩니다.)
3. 프로젝트 **Settings → Environment Variables**에서 새 변수를 추가합니다.
   - Key: `ADMIN_PASSWORD`
   - Value: 원하는 비밀번호 (다른 사람이 추측하기 어려운 것으로)
   - Environment: Production(및 필요 시 Preview)에 체크
4. **Settings → Deployments**에서 최신 배포를 **Redeploy**합니다. (환경변수는 재배포 후 적용됩니다.)
5. 이제 `/admin`에서 방금 설정한 비밀번호로 로그인하면 저장 기능이 정상 동작합니다.
...
Vercel이 자동으로 재배포합니다. 단, **관리자 페이지에서 저장한 문구는 git과 별개로 Vercel KV에
